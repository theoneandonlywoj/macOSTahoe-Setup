...
6. **Bash approval and allowlist learning**
   - Every Bash command prompts because `permission.bash` defaults to `"*": "ask"`.
   - After an approved command runs, ask whether to persist: (a) a non-dangerous first-token pattern such as `<first-token> *`, (b) the exact command string, or (c) nothing.
